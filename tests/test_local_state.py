...
class TestExtractionQueries:
    ...
    def test_get_extraction_details_not_exists(self, local_state_service, sample_coordinate):
        """Test get_extraction_details when path doesn't exist."""
        details = local_state_service.get_extraction_details(sample_coordinate)
        
        assert isinstance(details, ExtractionDetails)
        assert details.exists is False
        assert details.path is None
        assert details.subdirectories == []
        assert details.total_files == 0
        assert details.total_size == 0
        assert details.file_details == {}
    
    def test_get_extraction_details_with_files(self, local_state_service, sample_coordinate, temp_data_dirs):
        """Test get_extraction_details with actual files."""
        coord_path = local_state_service._build_coordinate_path(temp_data_dirs["processed_root"], sample_coordinate)
        coord_path.mkdir(parents=True)
        ...
        assert details.path == str(coord_path)
        assert sorted(details.subdirectories) == [
        ...
        assert details.total_size == 5750  # 1000 + 500 + 750 + 2000 + 1500
        ...
        assert set(details.file_details.keys()) == expected_files
...
class TestMLDataQueries:
    ...
    def test_check_ml_exported_with_data(self, local_state_service, sample_coordinate, temp_data_dirs):
        ...
        assert status.subdirectories == ["export_batch_001"]
        
        # Check sample files
        sample_files = status.sample_files
...
class TestHelperMethods:
    ...
    def test_count_files_in_path_with_error(self, local_state_service, temp_data_dirs):
        """Test _count_files_in_path with path that doesn't exist."""
        nonexistent_dir = temp_data_dirs["temp_dir"] / "nonexistent"
        
        counts = local_state_service._count_files_in_path(nonexistent_dir)
...
class TestRealFilesystemIntegration:
    ...
    def test_with_real_data_structure(self):
        ...
        if real_data_root.exists():
            ...
            # Test extraction details check
            extraction_details = service.get_extraction_details(coord)
